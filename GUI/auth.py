...
def signup():
    ...
    if user:
        if not user.is_active:
            return jsonify({'error': 'Account exists but not verified. Please login to receive a new OTP.'}), 400
...
def verify_otp():
    ...
    if datetime.now(timezone.utc) > otp_created_at + timedelta(minutes=5):
        return jsonify({'error': 'OTP expired. Please login to receive a new OTP.'}), 400
...
def login():
    ...
    if user and check_password_hash(user.password_hash, data['password']):
        if not user.is_active:
            ...
            db.session.commit()
            send_otp_email(mail, user.email, otp)
            return jsonify({'error': 'Account not verified. A new OTP has been sent to your email.'}), 403
        login_user(user)
